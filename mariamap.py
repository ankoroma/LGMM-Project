...
def get_recipe(item, idx):   
        ...
        fat_var = str(int(collection['hits'][idx]['recipe']['totalNutrients']['FAT']['quantity']) // size_var) + "g"
        carb_var = str(int(collection['hits'][idx]['recipe']['totalNutrients']['CHOCDF']['quantity']) // size_var) + "g"
        protein_var = str(int(collection['hits'][idx]['recipe']['totalNutrients']['PROCNT']['quantity']) // size_var) + "g"
        sugar_var = str(int(collection['hits'][idx]['recipe']['totalNutrients']['SUGAR']['quantity']) // size_var) + "g"
        sodium_var = str(int(collection['hits'][idx]['recipe']['totalNutrients']['CA']['quantity']) // size_var) + "mg"
...
query_prep_result = engine.execute("SELECT Name,Prep,Size,Ingredients FROM lean_green;").fetchall()
query_meal_result = engine.execute("SELECT Name,Meal,Dish,Health,Diet,Cuisine FROM lean_green;").fetchall()
query_nutrition_result = engine.execute("SELECT Name,Calories,Fat,Carbs,Protein,Sugar,Sodium FROM lean_green;").fetchall()
...
meal_df.columns = ['Name', 'Meal Type', 'Dish Type', 'Health Label', 'Diet Label', 'Cuisine']
